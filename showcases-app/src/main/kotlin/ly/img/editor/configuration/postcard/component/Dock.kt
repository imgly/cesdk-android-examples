...
class ColorsButtonBuilder : AbstractButtonBuilder<ColorsButtonScope>() {
    override var scope: ScopedProperty<EditorScope, ColorsButtonScope> = {
        remember(this) {
            ...
            val colorMapping = sortedMapOf<String, Pair<DesignBlock, Color>>()
            ...
            val colors = colorMapping.values.associate { (block, color) -> block to color }
            ColorsButtonScope(parentScope = this, colors = colors)
        }
    }

    private fun EditorContext.collectColorsRecursively(
        designBlock: DesignBlock,
        colorMapping: MutableMap<String, Pair<DesignBlock, Color>>,
    ) {
        val name = engine.block.getName(designBlock)
        if (name.isNotEmpty()) {
            ...
            color?.let { colorMapping[name] = designBlock to color }
        }

...
import ly.img.cesdk.dock.OptionIcon
...
internal fun createBlock(designBlock: DesignBlock, engine: Engine): Block {
    ...
    if ((type == DesignBlockType.IMAGE.key || type == DesignBlockType.STICKER.key) &&
        ...
        optionItems += OptionItemData(OptionType.Replace, R.string.cesdk_replace, OptionIcon.Vector(IconPack.Replace))
    }

    if (type == DesignBlockType.TEXT.key) {
        if (engine.block.isAllowedByScope(designBlock, "content/replace"))
            optionItems += OptionItemData(OptionType.Edit, R.string.cesdk_edit, OptionIcon.Vector(IconPack.Keyboard))
        if (engine.isStylingAllowed(designBlock))
            optionItems += OptionItemData(OptionType.Format, R.string.cesdk_format, OptionIcon.Vector(IconPack.Typeface))
    }
    ...
    if ((type == DesignBlockType.STAR_SHAPE.key || type == DesignBlockType.POLYGON_SHAPE.key || type == DesignBlockType.LINE_SHAPE.key) &&
        ...
        optionItems += OptionItemData(OptionType.ShapeOptions, R.string.cesdk_options, OptionIcon.Vector(IconPack.Tunevariant))
    }
    ...
    if (engine.isStylingAllowed(designBlock) || engine.isMoveAllowed(designBlock) ||
        ...
        optionItems += OptionItemData(OptionType.Layer, R.string.cesdk_layer, OptionIcon.Vector(IconPack.Layersoutline))
    }

    if (type == DesignBlockType.GROUP.key) {
        optionItems += OptionItemData(OptionType.EnterGroup, R.string.cesdk_enter_group, OptionIcon.Vector(IconPack.Groupenter))
    }

    if (engine.isGrouped(designBlock)) {
        optionItems += OptionItemData(OptionType.SelectGroup, R.string.cesdk_select_group, OptionIcon.Vector(IconPack.Selectgroup))
    }

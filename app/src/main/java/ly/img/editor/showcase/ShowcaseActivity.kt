...
import ly.img.editor.CreativeEditor
        ...
        setContent {
            EditorTheme {
                ...
                NavHost(navController = navController, startDestination = Screen.Showcases.routeScheme) {
                    ...
                    composable(
                        route = Screen.CreativeUi.routeScheme,
                    ) {
                        val scene = requireNotNull(it.arguments?.getString("0"))
                        if (scene == "default") {
                            CreativeEditor(
                                engineConfiguration = EngineConfiguration.getForCreative(license = Secrets.license),
                            ) {
                                navController.popBackStack()
                            }
                        } else {
...
sealed class Screen(val routeScheme: String) {
    ...
    data object CreativeUi : Screen("creative-ui?scene={0}")
}

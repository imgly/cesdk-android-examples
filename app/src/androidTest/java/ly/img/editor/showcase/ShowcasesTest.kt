...
@OptIn(ExperimentalTestApi::class, UnstableEngineApi::class)
class ShowcasesTest {
    ...
    fun before() {
        composeTestRule.runOnUiThread {
            ...
            Environment.getEngine().idlingEnabled = true
        }
    }
    ...
    fun snapApparelUi() {
        ...
        composeTestRule.waitUntilDoesNotExist(hasTestTag(testTag = "MainLoading"), timeoutMillis = 10_000)
        ...
        composeTestRule
            .onNodeWithTag(testTag = "LibraryImageCard/assets/demo/v2/ly.img.image/thumbnails/sample_1.jpg")
            .performClick()
    ...
    private fun capture(): Bitmap {
        ...
        val (windowBitmap, windowLocation) = activity.window.decorView.rootView.run {
        ...
        val (canvasBitmap, canvasLocation) = activity.findViewById<SurfaceView>(R.id.editor_surface_view).run {
            val bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
            val location = IntArray(2)
            getLocationInWindow(location)
        ...
        lock.await()
        val jointBitmap = Bitmap.createBitmap(windowBitmap.width, windowBitmap.height, Bitmap.Config.ARGB_8888)

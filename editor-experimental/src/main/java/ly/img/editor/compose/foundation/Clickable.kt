...
import ly.img.editor.compose.foundation.gestures.PressGestureScope
import ly.img.editor.compose.foundation.gestures.detectTapAndPress
    ...
    factory = {
        ...
        val gesture =
            Modifier.pointerInput(interactionSource, hasLongClick, hasDoubleClick, enabled) {
                ...
                detectTapGestures(
                    onDoubleTap =
                        if (hasDoubleClick && enabled) {
                            { onDoubleClickState.value?.invoke() }
                        } else {
                            null
                        },
                    onLongPress =
                        if (hasLongClick && enabled) {
...
internal fun Modifier.genericClickableWithoutGesture(
    ...
    fun Modifier.detectPressAndClickFromKey() =

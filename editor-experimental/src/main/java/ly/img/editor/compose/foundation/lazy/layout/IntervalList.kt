...
import ly.img.editor.compose.foundation.lazy.layout.IntervalList.Interval
...
class MutableIntervalList<T> : IntervalList<T> {
    private val intervals = mutableVectorOf<Interval<T>>()
     ...
     */
    private var lastInterval: Interval<T>? = null
    ...
    override fun get(index: Int): Interval<T> {
        checkIndexBounds(index)
        return getIntervalForIndex(index)
    }

    private fun getIntervalForIndex(itemIndex: Int): Interval<T> {
    ...
    private fun Interval<T>.contains(index: Int) = index in startIndex until startIndex + size
}
 ...
 */
private fun <T> MutableVector<Interval<T>>.binarySearch(itemIndex: Int): Int {
    var left = 0
    var right = lastIndex

 ...
 * @see verticalDrag
 */
suspend fun AwaitPointerEventScope.awaitVerticalDragOrCancellation(pointerId: PointerId): PointerInputChange? {
    if (currentEvent.isPointerUp(pointerId)) {
        return null // The pointer has already been lifted, so the gesture is canceled
    }
 ...
 * @see awaitDragOrCancellation
 */
suspend fun AwaitPointerEventScope.awaitHorizontalDragOrCancellation(pointerId: PointerId): PointerInputChange? {
    if (currentEvent.isPointerUp(pointerId)) {
        return null // The pointer has already been lifted, so the gesture is canceled
    }
...
internal suspend inline fun AwaitPointerEventScope.awaitPointerSlopOrCancellation(
    ...
    while (true) {
        ...
        if (dragEvent.isConsumed) {
            ...
            val mainPositionChange =
                pointerDirectionConfig.mainAxisDelta(currentPosition) -
                    pointerDirectionConfig.mainAxisDelta(previousPosition)

            val crossPositionChange =
                pointerDirectionConfig.crossAxisDelta(currentPosition) -
 ...
 */
internal val HorizontalPointerDirectionConfig =
    object : PointerDirectionConfig {
        override fun mainAxisDelta(offset: Offset): Float = offset.x

        override fun crossAxisDelta(offset: Offset): Float = offset.y

        override fun offsetFromChanges(
            mainChange: Float,
            crossChange: Float,
        ): Offset = Offset(mainChange, crossChange)
    }

/**
 * Used for monitoring changes on Y axis.
 */
internal val VerticalPointerDirectionConfig =
    object : PointerDirectionConfig {
        ...
        override fun offsetFromChanges(
            mainChange: Float,
            crossChange: Float,
        ): Offset = Offset(crossChange, mainChange)
    }
 ...
 */
suspend fun AwaitPointerEventScope.awaitLongPressOrCancellation(pointerId: PointerId): PointerInputChange? {
    if (currentEvent.isPointerUp(pointerId)) {

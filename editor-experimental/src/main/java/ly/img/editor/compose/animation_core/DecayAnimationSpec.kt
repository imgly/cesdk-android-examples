...
interface DecayAnimationSpec<T> {
     ...
     */
    fun <V : AnimationVector> vectorize(typeConverter: TwoWayConverter<T, V>): VectorizedDecayAnimationSpec<V>
}
...
private class DecayAnimationSpecImpl<T>(
    ...
    override fun <V : AnimationVector> vectorize(typeConverter: TwoWayConverter<T, V>): VectorizedDecayAnimationSpec<V> =
        VectorizedFloatDecaySpec(floatDecaySpec)
}
...
private class VectorizedFloatDecaySpec<V : AnimationVector>(
    ...
    override fun getValueFromNanos(
        ...
        for (i in 0 until valueVector.size) {
            valueVector[i] =
                floatDecaySpec.getValueFromNanos(
                    playTimeNanos,
                    initialValue[i],
                    initialVelocity[i],
                )

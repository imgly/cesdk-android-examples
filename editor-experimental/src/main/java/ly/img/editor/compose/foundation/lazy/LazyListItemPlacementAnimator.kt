...
internal class LazyListItemPlacementAnimator(
        ...
        itemInfo.placeables.fastForEachIndexed { index, placeableInfo ->
            ...
            if (currentTarget != currentOffset) {
                ...
                if (animationSpec != null) {
                    ...
                    scope.launch {
                        val finalSpec =
                            if (placeableInfo.animatedOffset.isRunning) {
                                // when interrupted, use the default spring, unless the spec is a spring.
                                if (animationSpec is SpringSpec<IntOffset>) {
                                    animationSpec
                                } else {
                                    InterruptionSpec
                                }
    ...
    private fun Int.toOffset() = IntOffset(if (isVertical) 0 else this, if (!isVertical) 0 else this)

...
import ly.img.editor.compose.animation.EnterExitState
...
private fun Modifier.shrinkExpand(
    ...
    composed {
        ...
        if (shouldAnimate) {
            ...
            val offsetAnimation =
                key(transition.currentState == transition.targetState) {
                    transition.createDeferredAnimation(
                        IntOffset.VectorConverter,
...
private class ExpandShrinkModifier(
    ...
    val offsetAnimation: Transition<EnterExitState>.DeferredAnimation<
        IntOffset,
        AnimationVector2D,
    >,
    ...
    override fun MeasureScope.measure(
        ...
        val measuredSize = IntSize(placeable.width, placeable.height)
        val currentSize =
            sizeAnimation.animate(sizeTransitionSpec) {
                sizeByState(it, measuredSize)
            }.value

        val offsetDelta =
            offsetAnimation.animate({ DefaultOffsetAnimationSpec }) {
